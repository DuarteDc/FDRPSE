...
final class SurveyService
{
    ...
    public function startSurvey()
    {
        if (!$this->surveyRepository->canStartNewSurvey()) {
            return new Exception(
                'Hay un cuestionatio en progreso por lo que no se puede comenzar la nueva encuesta',
                400
            );
        }
    ...
    public function saveNongradableAnswersByUser(array $body)
    {
        ...
        $isValidRequest = array_map(function ($question) use ($guideSurvey) {
            ...
            if ($question->type === TypeQuestion::SECTION->value) {
                ...
                return $section ? $this->validateSectionBinaryToSaveQuestion(
                    $section,
    ...
    public function finalzeUserSurvey()
    {
        ...
        return $surveyUser ? ['message' => 'La encuesta ha finalizado correctamente'] : new Exception(
            'Parece que hubo un error al finalizar la encuesta',
            500
        );
    }
