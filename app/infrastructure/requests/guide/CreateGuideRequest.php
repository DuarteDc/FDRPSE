...
final class CreateGuideRequest extends Request
{
    public static function rules(): array
    {
        return [
            ...
            'low' => 'numeric|min:' . (self::post('gradable') === 'gradable' ? (int) self::post(
                'despicable'
            ) + 1 : '0'),
            'middle' => 'numeric|min:' . (self::post('gradable') === 'gradable' ? (int) self::post(
                'low'
            ) + 1 : '0'),
            'high' => 'numeric|min:' . (self::post('gradable') === 'gradable' ? (int) self::post(
                'middle'
            ) + 1 : '0'),
            'very_high' => 'numeric|min:' . (self::post('gradable') === 'gradable' ? (int) self::post(
                'high'
            ) + 1 : '0'),
        ];
    }
    ...
    public static function messages(): array
    {
        return [
            ...
            'very_high:min' => 'La calificación más alta debe ser al menos de valor ' . (int) self::post(
                'high'
            ) + 1,
        ];
    }
